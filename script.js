...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Extend renderRoiSection with Payback Date & Export CSV
  function renderRoiSection() {
    let repayments = getRepaymentArr();
    let cashflows = [-roiInvestment, ...repayments];
    let cum = 0, payback = null;
    ...
    let paybackDateStr = 'n/a';
    if (payback !== null && loanStartDate) {
      const baseDate = new Date(loanStartDate);
      baseDate.setDate(baseDate.getDate() + (payback * 7));
      paybackDateStr = baseDate.toISOString().split('T')[0];
    }
    ...
    let summary = `<b>Total Investment:</b> €${roiInvestment.toLocaleString()}<br>
      ...
      <b>NPV (${roiInterest}%):</b> ${typeof npvVal === "number" ? "€"+npvVal.toLocaleString(undefined,{maximumFractionDigits:2}) : "n/a"}<br>
      <b>IRR:</b> ${isFinite(irrVal) && !isNaN(irrVal) ? (irrVal*100).toFixed(2)+'%' : 'n/a'}<br>
      <b>Payback (weeks):</b> ${payback ?? 'n/a'}<br>
      <b>Payback Date:</b> ${paybackDateStr}`;
    let roiSummary = document.getElementById('roiSummary');
    if (roiSummary) roiSummary.innerHTML = summary;
  ...
  // Export ROI KPIs to CSV
  function exportRoiKpisToCsv() {
    let repayments = getRepaymentArr();
    let cashflows = [-roiInvestment, ...repayments];
    let cum = 0, payback = null;
    for (let i = 1; i < cashflows.length; i++) {
    ...
    const csv = `Investment,Interest,NPV,IRR,PaybackWeek,PaybackDate\n` +
      `${roiInvestment},${roiInterest},${npvVal || ''},${irrVal ? (irrVal*100).toFixed(2)+'%' : ''},${payback ?? ''},${paybackDateStr}`;

    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.setAttribute("download", "roi_summary.csv");
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  }

  // Add button in DOM (must exist in HTML)
  const exportRoiBtn = document.getElementById('exportRoiCsvBtn');
  if (exportRoiBtn) {
    exportRoiBtn.onclick = exportRoiKpisToCsv;
  }
