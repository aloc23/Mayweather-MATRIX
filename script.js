...
// ===== NOVA SUB-TAB HELPER FUNCTIONS =====

/**
 * File upload functionality for Project Files tab
 */
function openFileUpload() {
  console.log('Opening file upload dialog');
  // Create a temporary file input if it doesn't exist
  let fileInput = document.getElementById('nova-file-upload');
  if (!fileInput) {
    fileInput = document.createElement('input');
    fileInput.type = 'file';
    fileInput.id = 'nova-file-upload';
    fileInput.multiple = true;
    fileInput.style.display = 'none';
    fileInput.addEventListener('change', handleNovaFileUpload);
...
/**
 * Handle file upload for Nova Project Files
 */
function handleNovaFileUpload(event) {
  const files = event.target.files;
  console.log(`Uploaded ${files.length} files:`, Array.from(files).map(f => f.name));
  
  // Update file counts in the UI
  const categories = document.querySelectorAll('#nova-panel-project-files .file-category p');
  categories.forEach((category, index) => {
    const currentCount = parseInt(category.textContent) || 0;
    category.textContent = `${currentCount + Math.ceil(files.length / categories.length)} files`;
  });
  
  // Show success message
  const placeholder = document.querySelector('#nova-panel-project-files .files-placeholder');
  if (placeholder) {
    placeholder.innerHTML = `
      <p>✓ Successfully uploaded ${files.length} files.</p>
      <p>Files are organized by category above.</p>
    `;
  }
}
...
function createNewScenario() {
  ...
  const scenarioList = document.querySelector('#nova-panel-scenarios .scenario-list .nova-kpi-card');
  if (scenarioList) {
    const scenarios = ['Best Case', 'Worst Case', 'Most Likely', 'Conservative'];
    const randomScenario = scenarios[Math.floor(Math.random() * scenarios.length)];
    
    scenarioList.innerHTML = `
      <h4>Available Scenarios</h4>
      <div class="scenario-item">
        <h5>${randomScenario} Scenario</h5>
        <p>Created: ${new Date().toLocaleDateString()}</p>
        <div class="scenario-metrics">
          <span>ROI: ${Math.floor(Math.random() * 25 + 5)}%</span>
          <span>Risk: ${Math.floor(Math.random() * 3 + 1)}/5</span>
        </div>
      </div>
      <ul class="scenario-features">
        <li>Best Case Analysis</li>
        <li>Worst Case Analysis</li>
        <li>Most Likely Case</li>
...
function compareScenarios() {
  ...
  alert('Scenario comparison functionality would open a detailed comparison view.');
}

// ===== NOVA INITIALIZATION =====

// Initialize Nova Enhanced Controller when DOM is ready
document.addEventListener('DOMContentLoaded', function() {
  // Wait a bit for all scripts to load
  setTimeout(() => {
    if (typeof NovaEnhancedController !== 'undefined') {
      window.novaController = new NovaEnhancedController();
      window.novaController.init();
